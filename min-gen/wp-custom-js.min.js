...
try{const response=await fetch(`/wp-content/json/${departure}/all.json`);const jsonData=await response.json();arrivalSelect.innerHTML='<option value="">도착지를 선택하세요</option>';let targetArr=[];let isArray=false;if(Array.isArray(jsonData)){targetArr=jsonData;isArray=true;}
else{targetArr=Object.keys(jsonData);}
targetArr.forEach(arrival=>{const option=document.createElement('option');option.value=arrival;option.textContent=arrival;arrivalSelect.appendChild(option);});if(isFirst){const links=document.getElementById("links");if(links){let newHTML=[];if(isArray){jsonData.forEach(departure=>{newHTML.push(`<a href="#"class="rounded-link"style="margin:0.2rem"data-departure="${data?.depature}">${departure}</a>`);});}
else{const now=new Date(new Date().toLocaleString('en-US',{timeZone:'Asia/Seoul'}));const currentHours=now.getHours();const currentMinutes=now.getMinutes();Object.keys(jsonData).forEach((key)=>{let value=jsonData[key];let isWorking=false;let departure=key;if(value&&"hours"in value&&value.hours){const[targetHours,targetMinutes]=value.hours.split(":").map(Number);if(currentHours<targetHours||(currentHours===targetHours&&currentMinutes<targetMinutes)){isWorking=true;}}
let classz=isWorking?"primary-link":"rounded-link";newHTML.push(`<a href="#"class="${classz}"style="margin:0.2rem"data-departure="${data?.depature}">${departure}</a>`);});}
const defaultDeparture=data?.depature||'';links.innerHTML=newHTML.join("");links.querySelectorAll(".rounded-link, .primary-link").forEach(link=>{link.addEventListener("click",async(event)=>{event.preventDefault();const departure=link.dataset.departure;const arrival=link.textContent;const response=await fetch(`/wp-content/json/${departure}/${arrival}/all.json`);const data=await response.json();if(data.url){window.location.href=data.url;}});});}
isFirst=false;}
