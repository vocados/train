...
try{const response=await fetch(`/wp-content/json/${departure}/all.json`);const jsonData=await response.json();arrivalSelect.innerHTML='<option value="">도착지를 선택하세요</option>';jsonData.forEach(arrival=>{const option=document.createElement('option');option.value=arrival;option.textContent=arrival;arrivalSelect.appendChild(option);});if(isFirst){const links=document.getElementById("links");if(links){let newHTML=[];jsonData.forEach(departure=>{newHTML.push(`<a href="#"class="rounded-link"style="margin:0.2rem"data-departure="${data?.depature}">${departure}</a>`);});const defaultDeparture=data?.depature||'';links.innerHTML=newHTML.join("");links.querySelectorAll(".rounded-link").forEach(link=>{link.addEventListener("click",async(event)=>{event.preventDefault();const departure=link.dataset.departure;const arrival=link.textContent;const response=await fetch(`/wp-content/json/${departure}/${arrival}/all.json`);const data=await response.json();if(data.url){window.location.href=data.url;}});});}
isFirst=false;}
